...
def clean_variable_name(name):
    ...
    name = name.replace("C(mechanism_clean)[T.", "")
    name = name.replace("C(state_clean)[T.", "")
    name = name.replace("C(hour_bin)[T.", "")
    name = name.replace("]", "")
...
top_plot = odds_ratios.sort_values("odds_ratio", ascending=False).head(20)

top_plot = top_plot.copy()
top_plot["clean_label"] = top_plot.index.map(clean_variable_name)
...
plt.hlines(
    y=y_pos,
    xmin=top_plot["lower_ci"],
    xmax=top_plot["upper_ci"],
    color="gray"
)
...
plt.yticks(y_pos, top_plot["clean_label"])

plt.xlabel("Odds Ratio (Severity Amplification)")
plt.title("Top Crash Severity Amplifiers")
